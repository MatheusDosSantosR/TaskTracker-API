...
export class AlterNameColumSubTask1729044040922 implements MigrationInterface {
    name = 'AlterNameColumSubTask1729044040922'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`ALTER TABLE \`subtask\` DROP COLUMN \`createdAt\``);
        await queryRunner.query(`ALTER TABLE \`subtask\` DROP COLUMN \`updatedAt\``);
        await queryRunner.query(`ALTER TABLE \`subtask\` ADD \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6)`);
        await queryRunner.query(`ALTER TABLE \`subtask\` ADD \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6)`);
        await queryRunner.query(`ALTER TABLE \`subtask\` ADD \`deleted_at\` datetime(6) NULL`);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE \`comment\` ADD CONSTRAINT \`FK_c0354a9a009d3bb45a08655ce3b\` FOREIGN KEY (\`userId\`) REFERENCES \`user\`(\`id\`) ON DELETE CASCADE ON UPDATE NO ACTION`);
        await queryRunner.query(`ALTER TABLE \`comment\` ADD CONSTRAINT \`FK_f28138baab6c22e4b27f489d8be\` FOREIGN KEY (\`todoId\`) REFERENCES \`todo\`(\`id\`) ON DELETE CASCADE ON UPDATE NO ACTION`);
        await queryRunner.query(`ALTER TABLE \`subtask\` DROP COLUMN \`deleted_at\``);
        await queryRunner.query(`ALTER TABLE \`subtask\` DROP COLUMN \`updated_at\``);
        await queryRunner.query(`ALTER TABLE \`subtask\` DROP COLUMN \`created_at\``);
        await queryRunner.query(`ALTER TABLE \`subtask\` ADD \`updatedAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6)`);
        await queryRunner.query(`ALTER TABLE \`subtask\` ADD \`createdAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6)`);
    }
